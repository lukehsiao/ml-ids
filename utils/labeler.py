...
def make_label_data():
    ...
    attack_files = [os.path.join(proj_dir, "data/master-listfile-condensed.txt")] * len(
        pcap_paths
    )
    #    attack_files = [os.path.join(proj_dir, 'data/master-listfile-testing.txt')]*len(pcap_paths)
    input_data = list(zip(pcap_paths, attack_files))
...
def label_packets(input_tuple):
    ...
    for dic in attacks:
        ...
        for elem, i in zip(attack_elems, list(range(num_pkts))):
            if elem == 1:
                attack_ids[i] = dic["ID"]
...
def checkIPsEqual(ip1, ip2):
    ...
    if len(ip1_vals) != 4 and len(ip2_vals) != 4:
        print("ERROR: checkIPsEqual: invalid IP address", file=sys.stderr)
        sys.exit(1)
    if ip1_vals[3] == "*" or ip2_vals[3] == "*":
        ip1_vals = list(map(int, ip1_vals[0:3]))
        ip2_vals = list(map(int, ip2_vals[0:3]))
    else:
        ip1_vals = list(map(int, ip1_vals))
        ip2_vals = list(map(int, ip2_vals))
